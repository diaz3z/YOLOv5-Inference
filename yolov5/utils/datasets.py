# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 16:13:58 2023
...
class LoadImages:
    ...
    def image_load(path, region_name,img_size, stride, auto=True):
        # img_size = img_size
        # stride = stride
        # auto = auto
        ...
        #im0 = Image.open(path)
        #print("Read PIL Image")
        # im0 = np.array(im0)
        # im0 = im0[:, :, ::-1] 
        if region_name == "LSTOP":
            im0 = im0[8:1574,115:1650]
            #print("Cropping done")
        #elif region_name == "ASBOTTOM":
            #im0 = im0[60:1749,88:2105]
            #im0 = cv2.convertScaleAbs(im0, alpha=1.5, beta=50)
            #print("Cropping done")
    ...
    # print("Reached LOADIMAGES")
    # # YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`
    # def __init__(self, path, img_size=640, stride=32, auto=True):
    #     #print("path before resolve",path)
    #     #start = time.time()
    #     #p = str(Path(path).resolve())  # os-agnostic absolute path
    #     #print(("Resolve time: ", time.time() - start)*1000)
    #     #print("path after resolve", p)
    #     # if '*' in p:
    ...
    #     print("opencv time: ",(time.time() - start_time2)*1000)
    #     start_time = time.time()
    #     img0 = Image.open(path)
    #     # pil_img0.draft('RGB',(640,640))  
       
    #     img0 = np.array(img0)        
    #     #print("Read PIL Image in LineScan")        
    #     img0 = img0[:, :, ::-1]
    #     print("Time for PIL loading: ", (time.time()-start_time)*1000)
    #     assert img0 is not None, f'Image Not Found {path}'
    #     start_time3 = time.time()
    #     # Padded resize
    #     img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0]
    #     print("Time for Padding: ", (time.time()-start_time3)*1000)
    #     # Convert
    #     start_time3 = time.time()
    #     img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
    #     # img = img.transpose((2, 0, 1)) # HWC to CHW
    #     img = np.ascontiguousarray(img)
    #     print("Time for transpose and ascontiguous : ", (time.time()-start_time3)*1000)
